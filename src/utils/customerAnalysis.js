...
export const formatDate = (value) =>
  dateKey(value) ? new Date(value).toLocaleDateString("th-TH", { timeZone: "Asia/Bangkok" }) : "–";
export const validSales = (sales) =>
  sales.filter((s) => s.status !== "cancelled" && dateKey(s.createdAt));
...
export function orderLines(sale, products = []) {
  ...
  const boxes = sale.boxes
    ? Object.values(sale.boxes)
    : [{ items: sale.items || [], boxQty: 1, legacy: true }];
    ...
    Object.values(box.items || []).forEach((item, index) => {
      ...
      if (!groups.has(key))
        groups.set(key, {
          ...
          name: item.productName || item.name || product.name || "ไม่ระบุ",
      ...
      if (!line.boxes.has(boxIndex)) {
        line.cartonQty += cartons;
        line.boxes.add(boxIndex);
      }
    });
  });
  return [...groups.values()].map(({ boxes, prices, ...line }) => ({
    ...line,
    unitPrice:
      prices.size === 1 ? [...prices][0] : line.quantity ? money(line.gross / line.quantity) : 0,
...
export function customerStats(sales, asOf = Date.now()) {
  const orders = validSales(sales)
    .filter((s) => dateKey(s.createdAt) <= dateKey(asOf))
  ...
  const first = orders[0].createdAt,
    last = orders[orders.length - 1].createdAt;
  ...
  const cycle =
    purchaseDays.length > 1
      ? Math.round(
          (Date.parse(purchaseDays[purchaseDays.length - 1]) - Date.parse(purchaseDays[0])) /
            DAY /
            (purchaseDays.length - 1),
        )
      : null;
  ...
  const sumYear = (y) =>
    money(
      orders
        .filter((s) => dateKey(s.createdAt).startsWith(String(y)))
        .reduce((sum, s) => sum + saleRevenue(s), 0),
    );
  const yearRevenue = sumYear(year),
    previousRevenue = sumYear(year - 1);
  return {
    orders,
    count: orders.length,
    revenue,
    average: money(revenue / orders.length),
    first,
    last,
    days,
    cycle,
    status: days > 180 ? "Lost" : days > 90 ? "At Risk" : "Active",
    yearRevenue,
    previousRevenue,
    growth: previousRevenue
      ? money(((yearRevenue - previousRevenue) / previousRevenue) * 100)
      : null,
  };
}
...
export function monthlySeries(sales, allSales, start, end) {
  ...
  while (`${year}-${String(month).padStart(2, "0")}` <= lastMonth) {
    ...
    rows.push({
      key,
      label: `${month}/${year + 543}`,
      value: money(current.reduce((sum, s) => sum + saleRevenue(s), 0)),
      previous: money(
        validSales(allSales)
          .filter((s) => inRange(s, prior(from), prior(to)))
          .reduce((sum, s) => sum + saleRevenue(s), 0),
      ),
      newCount,
      returning: buyers.length - newCount,
    });
    month++;
    if (month > 12) {
      month = 1;
  ...
  return validSales(sales)
    ...
    .flatMap((sale) => {
      ...
      return lines.map((line, index) => ({
        "Order Date": dateKey(sale.createdAt),
        "Order ID": orderCode(sale),
        "Invoice No": sale.invoiceNo || "",
        "Customer ID": customerCode(customer) || sale.customerId || "",
        "Customer Name": sale.customerName || customer.name || "",
        "Customer Type": customer.customerType || "ไม่ระบุ",
        Province: customer.province || "",
        Country: customer.country || "",
        "Acquisition Channel": customer.acquisitionChannel || "",
        "Sales Owner": sale.salesOwner || customer.salesOwner || "",
        SKU: line.sku,
        "Product Name": line.name,
        Quantity: line.quantity,
        Unit: line.unit,
        "Carton Qty": line.cartonQty,
        ...
        "Payment Method":
          sale.payment?.method === "cash"
            ? "เงินสด"
            : sale.payment?.method === "bank"
              ? "โอนธนาคาร"
              : "",
        "Payment Term": sale.paymentTerm || "",
        "Due Date": dateKey(sale.dueDate),
        "Paid Date": dateKey(sale.paidAt),
      }));
    });
}
