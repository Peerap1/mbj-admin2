...
beforeEach(() => {
  ...
  useBusinessData.mockReturnValue({
    loading: false,
    error: "",
    customers: [{ id: "c1", name: "ร้านทดสอบ", province: "ลำปาง", customerType: "ร้านของฝาก" }],
    products: [{ id: "p", sku: "SKU01", unit: "ถุง" }],
    sales: [
      {
        id: "o1",
        customerId: "c1",
        ...
        shippingCost: 100,
        boxes: [
          { boxQty: 2, items: [{ productId: "p", productName: "ข้าวแต๋น", qty: 3, price: 100 }] },
        ],
      },
    ],
  });
...
test("report exports selected order at SKU level with multiplied units", () => {
  ...
  const button = [...container.querySelectorAll("button")].find((b) =>
    b.textContent.includes("ส่งออก Excel"),
  );
  click(button);
...
test("clicking a ranked customer opens their profile and order history", () => {
  render();
  const button = [...container.querySelectorAll("button.analysis-bar")].find((b) =>
    b.textContent.includes("ร้านทดสอบ"),
  );
  click(button);
