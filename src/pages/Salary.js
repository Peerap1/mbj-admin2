...
function processWorkbook(wb) {
  ...
  wb.SheetNames.forEach((sheetName) => {
    ...
    parsed.forEach((row, idx) => {
      ...
      const dateKey = row._dt.toISOString().slice(0, 10);
      const key = `${name}|||${dateKey}`;
      if (!groups[key]) groups[key] = [];
      groups[key].push({ ...row, _rowIdx: idx });
    });

    // Determine highlight per row: red=<4, yellow=>4, none=4
    const highlights = {}; // rowIdx → "red"|"yellow"|null
    Object.values(groups).forEach((grp) => {
      const count = grp.length;
      let color = null;
      if (count < 4) color = "FF0000";      // red
      else if (count > 4) color = "FFFF00"; // yellow
      grp.forEach((r) => { highlights[r._rowIdx] = color; });
    });

    results[sheetName] = { rows: parsed, highlights, ws, groups };
  });

  return results;
}

// ─── Build output workbook with highlights ─────────────────────────
function buildOutputWorkbook(originalWb, processed) {
  ...
  originalWb.SheetNames.forEach((sheetName) => {
    ...
    // Build sheet from rows (without _dt/_rowIdx)
    const clean = rows.map(({ _dt, _rowIdx, ...rest }) => rest);

    const ws = XLSX.utils.json_to_sheet(clean);

    // Add cell styles (XLSX.js community edition doesn't support fillColor natively,
    // we encode highlight info as special comment / metadata in col D+)
    // Instead: we create a separate "ไฮไลท์" column showing the status
    const wsWithStatus = XLSX.utils.json_to_sheet(
      rows.map(({ _dt, _rowIdx, ...rest }, idx) => ({
        ...rest,
        สถานะ: highlights[idx] === "FF0000" ? "⚠ ไม่ครบ (<4)" :
               highlights[idx] === "FFFF00" ? "⚠ เกิน (>4)" : "✓ ปกติ",
      }))
    );

    XLSX.utils.book_append_sheet(outWb, wsWithStatus, sheetName);
  });
...
export default function Salary() {
  ...
  const handleDownload = () => {
    ...
    XLSX.writeFile(outWb, `${baseName}_highlight.xlsx`);
  };
